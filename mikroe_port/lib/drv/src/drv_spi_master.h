...
#include <stdint.h>
#include <stddef.h>
#include <string.h>     //eaf, may not be needed, memcpy warning
...
typedef struct
{
    ...
    spi_config config; //SPI Master Driver configuration structure
} spi_master_t;

void spi_master_configure_default(spi_config *config);                                                                                                                  //Configures #spi_master_config_t structure to default initialization values
uint8_t spi_master_open(spi_master_t *obj, spi_config *config);                                                                                                         //Opens the SPI Master driver object on selected pins
...
uint8_t spi_master_set_speed(spi_master_t *obj, spi_config speed);                                                                                                      //Sets SPI Master module speed to passed value if possible
uint8_t spi_master_set_mode(spi_master_t *obj, spi_config mode);                                                                                                        //Sets SPI Master module mode to passed value if possible
uint8_t spi_master_set_default_write_data(spi_master_t *obj, spi_config default_write_data);                                                                            //Default write data is sent by driver when the data transmit buffer is shorter than data receive buffer
uint8_t spi_master_write(spi_master_t *obj, uint8_t *write_data_buffer, size_t write_data_length);                                                                      //Writes byte to SPI bus in blocking mode                                        
uint8_t spi_master_read(spi_master_t *obj, uint8_t *read_data_buffer, size_t read_data_length);                                                                         //Reads byte from SPI bus in blocking mode                                             
uint8_t spi_master_write_then_read(spi_master_t *obj, uint8_t *write_data_buffer, size_t length_write_data, uint8_t *read_data_buffer, size_t length_read_data);        //Writes a sequence of bytes to SPI bus, followed by a corresponding read                                                                                      
